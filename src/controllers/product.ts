...
export const getSingleProduct = TryCatch(
    async (req, res, next) => {
        ...
        if (myCache.has(`product-${id}`)) product = JSON.parse(myCache
...
export const getAllProducts = TryCatch(
    async (req: Request<{}, {}, {}, SearchRequestQuery>, res, next) => {
        ...
        const productsPromise = Product.find(baseQuery)
            ...
            .limit(limit)
            .skip(skip)
        const [products, fliteredOnlyProduct] = await Promise.all([
            productsPromise, Product.find(baseQuery)
        ])
        const totalPage = Math.ceil(fliteredOnlyProduct.length / limit);
