...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-stats"))
        ...
        const latSixMonthOrdersPromise = Order.find({
            createdAt: {
                $gte: sixMonthsAgo,
                $lte: today,
            },
        ...
        latSixMonthOrders.forEach((order) => {
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-pie-chart"))
        ...
        const marketingCost = Math.round(grossIncome - (productionCost + burnt));
